...
CanScriptReader::CanScriptReader(const std::string &scriptFilePath) :
    m_scriptFilePath{scriptFilePath},
    m_commands{std::make_shared<std::vector<CanCommand>>()}
{
        ...
        try {
            ...
            if (foundLoopPosition != std::string::npos) {
                ...
                if (trimWhitespace(targetLoopCount) == "") {
                    loop = true;
                    loopCount = -1;
                    this->m_commands->emplace_back(CanCommandType::LOOP_START, std::to_string(loopCount));
                } else {
                    try {
                        loopCount = tAbs(std::stoi(targetLoopCount));
                        loop = true;
                        this->m_commands->emplace_back(CanCommandType::LOOP_START, targetLoopCount);
                ...
                if (!loop) {
                    ...
                    loop = false;
                    this->m_commands->emplace_back(CanCommandType::LOOP_END, "");
                }
                ...
                try {
                    ...
                    std::cout << GENERIC_CONFIG_WARNING_BASE_STRING << currentLine << GENERIC_CONFIG_WARNING_TAIL_STRING << std::endl;
                    if (commandType == CanCommandType::DELAY_SECONDS) {
                        std::cout << DELAY_SECONDS_PARAMETER_NOT_AN_INTEGER_STRING << std::endl;
                    } else if (commandType == CanCommandType::DELAY_MILLISECONDS) {
                        std::cout << DELAY_MILLISECONDS_PARAMETER_NOT_AN_INTEGER_STRING << std::endl;
                    } else if (commandType == CanCommandType::DELAY_MICROSECONDS) {
                ...
                this->m_commands->emplace_back(CanCommandType::WRITE, targetString);
                ...
                this->m_commands->emplace_back(CanCommandType::READ, "");
                ...
                this->m_commands->emplace_back(CanCommandType::FLUSH_RX, "");
                ...
                this->m_commands->emplace_back(CanCommandType::FLUSH_TX, "");
                ...
                this->m_commands->emplace_back(CanCommandType::FLUSH_RX_TX, "");
            } else {
                std::cout << GENERIC_CONFIG_WARNING_BASE_STRING << currentLine << GENERIC_CONFIG_WARNING_TAIL_STRING << std::endl;
                std::cout << CONFIG_EXPRESSION_MALFORMED_STRING << std::endl;
                std::cout << *iter << std::endl;
...
std::shared_ptr<std::vector<CanCommand>> CanScriptReader::commands() const
{
